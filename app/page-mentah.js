...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center">
        <h1 className="text-2xl font-semibold mb-4">✨ Today’s Zodiac Reading</h1>
      ...
      <div className="perspective w-[320px] h-[320px]">
        ...
        <div className={`relative w-full h-full duration-700 transform-style ${showCard ? "rotate-y-180" : ""}`}>
          ...
          {/* FRONT (WHEEL) */}
          <div className="absolute w-full h-full backface-hidden flex items-center justify-center border border-yellow-400 rounded-2xl">
            
            <p className="text-yellow-400 text-lg">
              {selected ? selected.toUpperCase() : "Pick your zodiac"}
            </p>

          </div>

          {/* BACK (RESULT) */}
           <div className="absolute w-full h-full backface-hidden rotate-y-180 bg-white text-black p-4 rounded-xl flex flex-col justify-center">
              {result && (
              <>
                <p>💕 {result.love}</p>
                <p>🌱 {result.life}</p>
      ...
      {/* ✅ HIDE WHEEL SAAT FLIP */}
        {!showCard && (
          <>
            {zodiacList.map((z, i) => {
              const pos = getPosition(i, zodiacList.length, 120);
      ...
      {!showCard ? (
        <button
          ...
          disabled={!selected}
          className="mt-6 border border-yellow-400 px-5 py-2 rounded-full"
        >
          see today’s reading ✨
        </button>
      ) : (
        <button
          onClick={handleBack}
          className="mt-6 border border-white px-5 py-2 rounded-full"
        >
          ← back
        </button>
      )}
